# =============================================================================
# pp3a_GaussI.py - Nội suy Gauss tiến (Gauss Forward)
#
# Chức năng: Nội suy bằng công thức Gauss tiến dùng sai phân trung
#            tâm (centered differences) cho các điểm cách đều.
#
# Các hàm chính:
#   EvenDifference(points)              - bảng sai phân
#   gauss_1_interpolation(points, x0_index) - công thức Gauss tiến
#   all_derivatives(coeffs, t, h)      - đạo hàm
#
# Cách dùng: python pp3a_GaussI.py
# =============================================================================
import numpy as np
import pandas as pd
import math
...
def gauss_1_interpolation(points, x0_index):
    """
    Constructs the Gauss I interpolation polynomial in the variable t = (x - x0) / h.
    ...
    n = len(points) - 1
    if (n + 1) % 2 == 0:
        raise ValueError("Gauss I formula requires an odd number of nodes (even degree n).")
    ...
    # --- 2. Select Coefficients (Gauss I Path) ---
    C_coeffs = []
    for i in range(n + 1):
        order_col = f'Order {i}' if i > 0 else 'y_i'
        row_index = x0_index - (i // 2)
...
df.style
# 3. Calculate the Gauss I polynomial
step_df, final_coeff_df = gauss_1_interpolation(points, x0_index)

print("\n--- Polynomial Construction Steps (Gauss I) ---")
step_df.style
