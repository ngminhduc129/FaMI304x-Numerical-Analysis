...
1. Khởi tạo: gán $\text{result} = c_n$ (hệ số bậc cao nhất).
2. Với mỗi $i$ từ $n-1$ xuống $0$:
   - $\text{result} = \text{result} \cdot x + c_i$.
3. Trả về $\text{result}$.

### `polynomial_derivative(coeffs, x)`
...
1. Xác định bậc đa thức: $n = \text{len(coeffs)} - 1$.
2. Lấy hệ số bậc cao nhất: $c_n = \text{coeffs}[n]$.
3. Nếu $c_n = 0$: báo lỗi "Hệ số bậc cao nhất không được bằng $0$".
4. Tìm giá trị tuyệt đối lớn nhất của các hệ số từ $c_0$ đến $c_{n-1}$:
   - $\text{max\_abs} = \max(|c_0|, |c_1|, \dots, |c_{n-1}|)$.
5. Tính $R = 1 + \dfrac{\text{max\_abs}}{|c_n|}$.
6. Trả về $R$.
...
1. Xác định bậc đa thức: $n = \text{len(coeffs)} - 1$.
2. Lấy $c_n = \text{coeffs}[n]$.
3. Duyệt qua các hệ số $c_0, c_1, \dots, c_{n-1}$:
   - Lọc ra các hệ số âm.
   - Nếu không có hệ số âm nào: trả về $\text{None}$.
4. Tìm $B$ là giá trị tuyệt đối lớn nhất trong các hệ số âm.
5. Tìm $k$ là chỉ số lớn nhất của hệ số âm có giá trị tuyệt đối bằng $B$.
6. Tính $R = 1 + \left(\dfrac{B}{|c_n|}\right)^{1/(n-k)}$.
7. Trả về $R$.

### `find_extrema(coeffs, lower_bound, upper_bound, lr, tolerance)`

Tìm cực trị địa phương bằng gradient descent/ascent với learning rate thích ứng.

**Thuật toán:**

**Đầu vào:** Hệ số đa thức $\text{coeffs} = [c_0, c_1, \dots, c_n]$, cận dưới $\text{lower\_bound}$, cận trên $\text{upper\_bound}$,
tốc độ học $\text{lr}$, ngưỡng hội tụ $\text{tolerance}$.
**Đầu ra:** Danh sách các điểm cực trị $(x, P(x), \text{loại})$.

1. Khởi tạo tập điểm xuất phát:
   - Chia đoạn $[\text{lower\_bound}, \text{upper\_bound}]$ thành $N$ điểm cách đều.
   - Gọi $\text{points}$ là danh sách các điểm xuất phát này.

2. Với mỗi điểm $p$ trong $\text{points}$:
   a. Gán $x = p$.
   b. Lặp:
...
5. Trả về danh sách các điểm cực trị $(x, P(x), \text{loại})$.
