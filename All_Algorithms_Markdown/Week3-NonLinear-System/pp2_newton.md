...
## Thuật toán `newton_method`

**Mục tiêu:** Tìm nghiệm xấp xỉ của hệ phương trình phi tuyến bằng phương pháp Newton.
**Đầu vào:**
- `initial_values`: vector khởi tạo $\mathbf{x}^{(0)} = (x_1^{(0)}, x_2^{(0)}, \dots, x_n^{(0)})^T$
- `tol`: sai số cho phép (mặc định $10^{-6}$)
- `max_iter`: số lần lặp tối đa (mặc định 100)
- `*funcs`: danh sách các hàm $f_1, f_2, \dots, f_n$ và hàm Jacobian

**Đầu ra:** Nghiệm xấp xỉ $\mathbf{x}^*$ của hệ phương trình.

**Bước 1:** Khởi tạo giá trị lặp ban đầu và bảng kết quả
- Gán $\mathbf{x} = \text{initial\_values}$, đây là $\mathbf{x}^{(0)}$.
- Tạo bảng để lưu kết quả từng bước lặp, bao gồm: số thứ tự lần lặp, vector $\mathbf{x}$ hiện tại, vector $\Delta\mathbf{x}$, chuẩn của $\Delta\mathbf{x}$ và vector giá trị hàm $\mathbf{F}(\mathbf{x})$.

**Bước 2:** Với mỗi lần lặp $i = 0, 1, 2, \dots$ (tối đa `max_iter` lần)
   ...
   **Bước 2.2:** Tính ma trận Jacobian $\mathbf{J}(\mathbf{x})$
   - Gọi hàm Jacobian để tính ma trận đạo hàm riêng cấp 1 tại $\mathbf{x}$.
   ...
   **Bước 2.4:** Giải hệ phương trình tuyến tính
   - Sử dụng khử Gauss, phân rã LU, hoặc các phương pháp giải hệ tuyến tính khác để tìm $\Delta\mathbf{x}$.

   **Bước 2.5:** Cập nhật nghiệm xấp xỉ
   - Tính $\mathbf{x}_\text{new} = \mathbf{x} + \Delta\mathbf{x}$.
   - Cụ thể, với mỗi $j = 1, 2, \dots, n$: $x_{\text{new}, j} = x_j + \Delta x_j$.

   **Bước 2.6:** Tính chuẩn 1 của vector chênh lệch
   - $\text{norm\_diff} = \|\mathbf{x}_\text{new} - \mathbf{x}\|_1 = \sum_{j=1}^{n} |\Delta x_j|$.

   **Bước 2.7:** Lưu kết quả lần lặp hiện tại vào bảng.

   **Bước 2.8:** Cập nhật giá trị lặp: $\mathbf{x} = \mathbf{x}_\text{new}$.

   **Bước 2.9:** Kiểm tra điều kiện dừng
   - Nếu $\text{norm\_diff} < \text{tol}$: kết thúc vòng lặp.
   - Nếu $i = \text{max\_iter} - 1$: kết thúc vòng lặp (tránh lặp vô hạn).
   - Ngược lại, tăng $i$ lên 1 và quay lại Bước 2.1.

**Bước 3:** In bảng kết quả.

**Bước 4:** Trả về nghiệm xấp xỉ $\mathbf{x}^* = \mathbf{x}$.
