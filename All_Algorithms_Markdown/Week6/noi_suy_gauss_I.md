# Nội suy Gauss tiến (Gauss Forward Interpolation)

## Mô tả
Nội suy bằng công thức Gauss tiến dùng sai phân trung tâm (centered differences) cho các điểm cách đều. Phù hợp khi điểm nội suy gần tâm của tập dữ liệu.

## Công thức toán học

**Công thức Gauss I (Gauss tiến)**:

Đặt $t = \frac{x - x_0}{h}$:

$$P_n(x) = y_0 + t\Delta y_0 + \frac{t(t-1)}{2!}\Delta^2 y_{-1} + \frac{t(t-1)(t+1)}{3!}\Delta^3 y_{-1} + \frac{t(t-1)(t+1)(t-2)}{4!}\Delta^4 y_{-2} + \dots$$

Tổng quát, các hệ số được chọn theo quy tắc: $y_0, \Delta y_0, \Delta^2 y_{-1}, \Delta^3 y_{-1}, \Delta^4 y_{-2}, \Delta^5 y_{-2}, \dots$
...
1. **Khởi tạo bảng:** Tạo bảng sai phân hai chiều với $n+1$ hàng và $n+1$ cột.
   ...
   b. $x_0$ là điểm trung tâm (gần $x$ nhất).
...
2. **Duyệt $i$ từ 0 đến $n$:**
   a. Với $i$ chẵn ($i = 2k$):
      - Hàng: $x_0 - k$, tức là $x_0 - \lfloor i/2 \rfloor$.
      - Cột: $i$.
      - Hệ số: $\Delta^i y_{-k}$.
   b. Với $i$ lẻ ($i = 2k+1$):
      - Hàng: $x_0 - k$, tức là $x_0 - \lfloor i/2 \rfloor$.
   ...
   c. Cụ thể:
      ...
      - $i = 1$: $\Delta y_0$ (hàng 0, cột 1).
      - $i = 2$: $\Delta^2 y_{-1}$ (hàng -1, cột 2).
      - $i = 3$: $\Delta^3 y_{-1}$ (hàng -1, cột 3).
...
2. **Với mỗi $i$ từ 0 đến $n$:**
   a. **Lấy hệ số $C_i$** từ bảng theo quy tắc Gauss I.
...
3. **Kết quả:** $P_n(x)$ là giá trị nội suy Gauss tiến tại $x$.
